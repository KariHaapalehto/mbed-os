...
#ifndef MBED_CONF_MBED_MESH_API_WS_DEFAULT_NETWORK_NAME
#error "please define network name for Wi-SUN"
#endif
...
#include "Platforms/simulator/arm_hal_timer.h"
#else
// Dummy FHSS functions for HW, will be replaced by FHSS drivers later. These are needed to start the WS bootstrap.
...
static int platform_fhss_timer_start(void (*callback)(const fhss_api_t *api, uint16_t), const fhss_api_t *api)
{
    (void)callback;
    (void)api;
    return 0;
...
static fhss_timer_t fhss_functions = {
...
static char* network_name = MBED_CONF_MBED_MESH_API_WS_DEFAULT_NETWORK_NAME;
...
static void wisun_tasklet_configure_and_connect_to_network(void)
{
    ...
    wisun_tasklet_data_ptr->channel_list.channel_page = (channel_page_e)MBED_CONF_MBED_MESH_API_THREAD_CONFIG_CHANNEL_PAGE;
    wisun_tasklet_data_ptr->channel_list.channel_mask[0] = MBED_CONF_MBED_MESH_API_THREAD_CONFIG_CHANNEL_MASK;
    
    tr_debug("channel page: %d", wisun_tasklet_data_ptr->channel_list.channel_page);
    tr_debug("channel mask: 0x%.8lx", wisun_tasklet_data_ptr->channel_list.channel_mask[0]);

    ws_management_node_init(wisun_tasklet_data_ptr->network_interface_id,network_name, fhss_timer_ptr);

    if (MBED_CONF_MBED_MESH_API_WS_CHANNEL_FUNCTION != 255) {
        ret = ws_management_fhss_unicast_channel_function_configure(wisun_tasklet_data_ptr->network_interface_id,
                                                                    MBED_CONF_MBED_MESH_API_WS_CHANNEL_FUNCTION,
                                                                    MBED_CONF_MBED_MESH_API_WS_DEFAULT_FIXED_CHANNEL,
                                                                    MBED_CONF_MBED_MESH_API_WS_DEFAULT_DWELL_TIME);
        if (ret != 0) {
        	tr_error("Channel function configuration failed %"PRIi32"\r\n", ret);
        }
    }

    if (MBED_CONF_MBED_MESH_API_WS_FHSS_BC_INTERVAL != 0 ||
        MBED_CONF_MBED_MESH_API_WS_FHSS_BC_DWELL_INTERVAL != 0 ||
        MBED_CONF_MBED_MESH_API_WS_FHSS_UC_DWELL_INTERVAL != 0 ) {
        ret = ws_management_fhss_timing_configure(wisun_tasklet_data_ptr->network_interface_id,
                                                  MBED_CONF_MBED_MESH_API_WS_FHSS_UC_DWELL_INTERVAL,
                                                  MBED_CONF_MBED_MESH_API_WS_FHSS_BC_INTERVAL,
                                                  MBED_CONF_MBED_MESH_API_WS_FHSS_BC_DWELL_INTERVAL);
        if (ret != 0) {
        	tr_error("fhss configuration failed %"PRIi32"\r\n", ret);
        }
    }

    if (MBED_CONF_MBED_MESH_API_WS_REGULATOR_DOMAIN != 255 ||
        MBED_CONF_MBED_MESH_API_WS_OPERATING_MODE != 255 ||
        MBED_CONF_MBED_MESH_API_WS_OPERATING_CLASS != 255 ) {
        ret = ws_management_regulatory_domain_set(wisun_tasklet_data_ptr->network_interface_id,
                                                  MBED_CONF_MBED_MESH_API_WS_REGULATOR_DOMAIN,
                                                  MBED_CONF_MBED_MESH_API_WS_OPERATING_CLASS,
                                                  MBED_CONF_MBED_MESH_API_WS_OPERATING_MODE);
        if (ret != 0) {
        	tr_error("Regulatory domain configuration failed %"PRIi32"\r\n", ret);
        }
    }
...
void wisun_tasklet_trace_bootstrap_info()
{
    ...
    if (arm_nwk_nd_address_read(wisun_tasklet_data_ptr->network_interface_id,
        ...
        if (arm_net_address_get(wisun_tasklet_data_ptr->network_interface_id,
                                ADDR_IPV6_GP, temp_ipv6) == 0) {
            tr_debug("GP IPv6: %s", trace_ipv6(temp_ipv6));
        }
    }

    if (arm_nwk_mac_address_read(wisun_tasklet_data_ptr->network_interface_id,
...
int8_t wisun_tasklet_get_router_ip_address(char *address, int8_t len)
{
    ...
    if ((len >= 40) && (0 == arm_nwk_nd_address_read(
                            wisun_tasklet_data_ptr->network_interface_id, &nd_address))) {
...
int8_t wisun_tasklet_connect(mesh_interface_cb callback, int8_t nwk_interface_id)
{
    ...
    int32_t ret = ws_management_node_init(nwk_interface_id, network_name, fhss_timer_ptr);
    if (0 != ret) {
    	tr_debug("\r WS node init fail - code %"PRIi32"\r\n", ret);
        return -1;
    }
...
void wisun_tasklet_init(void)
{
    if (wisun_tasklet_data_ptr == NULL) {
        ...
        wisun_tasklet_data_ptr->network_interface_id = INVALID_INTERFACE_ID;
        wisun_tasklet_data_ptr->operating_mode = MBED_CONF_MBED_MESH_API_WS_DEVICE_TYPE;
...
int8_t wisun_tasklet_network_init(int8_t device_id)
{
    ...
    return arm_nwk_interface_lowpan_init(mac_api, MBED_CONF_MBED_MESH_API_WS_DEFAULT_NETWORK_NAME);
}
